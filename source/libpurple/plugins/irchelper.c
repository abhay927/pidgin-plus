...
#define BOUNCED_DOMAIN_PREFERENCE PLUGIN_ID "_bounced_domain"
...
typedef enum {
	...
	IRC_NETWORK_TYPE_BOUNCED     = 0x10000,
} IRCHelperStateFlags;
...
static IRCHelperStateFlags get_connection_type(PurpleConnection *connection)
{
	...
	bounced_domain = purple_account_get_string(account, BOUNCED_DOMAIN_PREFERENCE, "");
	bounced = (NULL != bounced_domain && '\0' != *bounced_domain);
	if (bounced)
		username = g_strconcat(username, "=(bounced).", bounced_domain, NULL);
	...
	if (bounced)
		type = (type | IRC_NETWORK_TYPE_BOUNCED);
...
static void signed_on_cb(PurpleConnection *connection)
{
	...
	if (state & IRC_NETWORK_TYPE_BOUNCED)
		purple_debug_info(PLUGIN_STATIC_NAME, "assuming account %s as a bouncer to %s\n",
			g_utf8_strdown(purple_account_get_username(account), -1),
			purple_account_get_string(account, BOUNCED_DOMAIN_PREFERENCE, ""));
	...
	else
	{
		...
		if ('\0' != *nickpassword || (state & IRC_NETWORK_TYPE_BOUNCED))
		{
			char **userparts;

			g_hash_table_insert(states, connection->proto_data,
			                    GINT_TO_POINTER((state & IRC_NETWORK_TYPE_BOUNCED) | IRC_NETWORK_TYPE_NICKSERV | IRC_WILL_ID));
			    ...
			    strcmp(userparts[0], purple_connection_get_display_name(connection)))
			{
				...
				g_hash_table_insert(states, connection->proto_data,
				                    GINT_TO_POINTER((state & IRC_NETWORK_TYPE_BOUNCED) | IRC_NETWORK_TYPE_NICKSERV | IRC_KILLING_GHOST));
...
static gboolean plugin_load(PurplePlugin *plugin)
{
	...
	option = purple_account_option_string_new(_("Bounced domain"), BOUNCED_DOMAIN_PREFERENCE, "");
	prpl_info->protocol_options = g_list_append(prpl_info->protocol_options, option);
