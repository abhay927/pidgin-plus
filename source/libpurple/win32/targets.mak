...
$(PIDGIN_CONFIG_H): $(PIDGIN_CONFIG_H).mingw $(PIDGIN_TREE_TOP)/configure.ac
	sed -e 's/@VERSION@/$(APPLICATION_VERSION)/; s/@DISPLAY_VERSION@/$(DISPLAY_VERSION)/' $@.mingw > $@
...
$(PIDGIN_REVISION_RAW_TXT):
	(bzr version-info --custom --template "{branch_nick}.{revno}" "$(BAZAAR_BRANCH)") 2>/dev/null >$@ \
	|| rm -f $@
